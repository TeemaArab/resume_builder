...
import { BriefcaseBusiness, User, MapPin, Phone, Mail, Globe , Linkedin} from 'lucide-react'
          ...
          {typeof data.image ==='object'&&(
            <div className='flex flex-col gap-1 pl-4 text-sm'>
                ...
                <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                     ...
                     rounded-full transition-transform duration-200 ease-in-out peer-checked:translate-x-4'></span>
                </label>
            </div>
          )}
       </div>
       {/* display list of input fields  */}
       {fields.map((field) =>{
         ...
         return(
            <div key={field.key} className='space-y-1 mt-5'>
               ...
               <input type={field.type} value={data[field.key] || ''} onChange={(e) => handleChange(field.key, e.target.value)} className='mt-1
                w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm'
