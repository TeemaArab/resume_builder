...
const TemplateSelector = ({selectedTemplate, onChange}) => {
    ...
    const templates = [
        ...
        {id: 'modern', name: 'Modern', preview: "A sleek, contemporary design with bold headings and a focus on visual hierarchy"},
        {id: 'minimal', name: 'Minimal', preview: "A simple, elegant layout with ample white space and understated typography"},
        {id: 'minimal-image', name: 'Minimal with Image', preview: "A minimal design that includes a profile image for a personal touch"},
    ];
  return (
       ...
       {isOpen &&(
        ...
        {templates.map((template) => (
            <div key={template.id} onClick={()=>{onChange(template.id);
                setIsOpen(false) }} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 'border-blue-400  bg-blue-100' 
