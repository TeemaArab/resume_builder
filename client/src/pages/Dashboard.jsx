...
const Dashboard = () => {
  ...
  const loadAllResumes = async()=>{
    ...
    if (!token) return; // ADDED FOR DEBUGGING PURPOSES
     try{
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
       <p className='text-2xl font-medium mb-6 bg-gradient-to-r from slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome to your dashboard, {user.name}</p>
       ...
       <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume,index)=>{
            ...
            return(
              ...
              <button key={index} onClick ={()=>navigate(`/app/builder/${resume._id}`)} className= ' relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border border group hover:shadow-lg transition-all duration-300 cursor-pointer'
       ...
       {showUploadResume &&(
        <form  onSubmit={uploadResume} onClick={()=>setShowUploadResume(false)} className='fixed inset-0 bg-black/70  backdrop-blur bg-opacitiy-50 z-10 flex items-center justify-center'>
             <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button  disabled ={isLoading} className='w-full py-2 bg-purple-600 text-white rounded 
